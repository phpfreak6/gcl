    ...
    <section class="section dashboard">
        ...
        <div class="card-body mt-5 table-responsive bg-white py-4 rounded">
            <div class="w-100 d-flex justify-content-end mb-2 border-bottom" >
                ...
                <a href="{{route('couriers.add')}}" class="ms-auto my-1 btn btn-primary text-white">Add New Courier</a>
               </div>
            <table id="courierTable" class="table table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><img src="{{ asset($courier->logo) }}" alt="Logo" style="max-height: 50px;"></td>
                        <td>{{ $courier->courier_name }}</td>
                        <td>{{ $courier->auth_company }}</td>
                        <td>{{ $courier->short_description }}</td>
                        <td>
                            <a href="#" class="btn btn-info text-white view-btn" data-toggle="modal" data-target="#viewModal" data-details="{{ json_encode($courier) }}">View</a>
                            <a href="{{ route('courier-services.index', $courier->id) }}" class="btn btn-secondary">Services</a>
                            {{-- <a href="{{ route('courier.services', $courier->id) }}" class="btn btn-secondary">Services</a> --}}
                            <a href="{{ route('couriers.edit', $courier->id) }}" class="btn btn-primary">Edit</a>
<button class="btn btn-danger delete-btn" data-id="{{ $courier->id }}">Delete</button>
    ...
    $('.view-btn').on('click', function () {
        ...
        $.each(details, function(key, value) {
            if(key !== 'logo') { // Exclude the logo from text details
                ...
                modalBody.append('<img src="{{ asset('') }}' + value + '" alt="Logo" style="max-width: 100%;">');
            }
        });
